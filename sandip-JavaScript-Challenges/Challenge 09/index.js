...
const reformattedArray = users.map(({ displayName }) => ({ displayName }));
console.log(reformattedArray);


const SrFrontEndDeveloper = users.filter((user) => user.jobtitle.includes('Sr. Front-End Engineer'));
console.log(SrFrontEndDeveloper);


const average = users.reduce((number, user) => number + user.employeeNumber, 0);
console.log(average);


const teamNumbers = SrFrontEndDeveloper.map(({ displayName }) => ({ displayName }));
